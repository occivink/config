...
add-highlighter -group / group gdb
...
hook global WinCreate .* %{
    add-highlighter ref gdb
}

# user-facing commands
define-command gdb-connect %{
    gdb-stop
    %sh{
...
define-command gdb-run              %{ gdb-cmd -exec-run }
define-command gdb-start            %{ gdb-cmd "-exec-run --start" }
...
define-command gdb-until            %{ gdb-cmd "-exec-until %val{buffile}:%val{cursor_line}" }
define-command gdb-set-breakpoint   %{ gdb-cmd "-break-insert %val{buffile}:%val{cursor_line}" }
