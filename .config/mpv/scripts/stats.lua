...
local o = {
    ...
    -- Text style
    font = "sans-serif",
    font_mono = "mono",   -- monospaced digits are sufficient
    font_size = 8,
    font_color = "FFFFFF",
    border_size = 0.8,
    border_color = "262626",
...
local function append_display_sync(s)
    ...
    if toggled_active and (o.plot_vsync_ratio or o.plot_vsync_jitter) and o.use_ass then
        local ratio_graph = ""
        local jitter_graph = ""
...
local function print_page(page)
    if o.persistent_overlay then
        ...
        mp.osd_message(pages[page].f(), o.redraw_delay + 1)
    end
end
...
local oneshot_should_print = false
local toggled_active = false


local function print_current_page()
    if not oneshot_should_print then return end
    print_page(curr_page)
    oneshot_should_print = false
end


local function oneshot(table)
    if toggled_active then return end

    local event = table["event"]
    if event == "up" then
        mp.unregister_idle(print_current_page)
        oneshot_should_print = false
    elseif event == "down" then
        mp.register_idle(print_current_page)
        oneshot_should_print = true
        print_current_page()
...
display_timer = mp.add_periodic_timer(o.duration, function() print_page(curr_page) end)
display_timer:kill()


local function toggle()
    -- unregister idle event in the unlikely event it was on
    mp.unregister_idle(print_current_page)

    if toggled_active then
        for k, _ in pairs(pages) do
            mp.remove_key_binding("stats-" .. k)
        end
        ...
        for k, _ in pairs(pages) do
            mp.add_forced_key_binding(k, "stats-" .. k, function()
                curr_page = k
                print_page(k)
            end)
        end
...
mp.add_key_binding(o.key_oneshot, "display-stats", oneshot, {complex=true})
mp.add_key_binding(o.key_toggle, "display-stats-toggle", toggle)
